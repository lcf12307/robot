...
// 获取自动给图片增加文字结果
func getPsResult(name, text string) string {
	...
	return handleImg(url, true).(string)
}

func handleImg(url string, isM bool) interface{} {
	if isM {
		picTmp := "![%s](%s)"
		return fmt.Sprintf(picTmp, "", url)
	}
